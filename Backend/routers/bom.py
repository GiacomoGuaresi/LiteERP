...
def create_billOfMaterials(item: BillOfMaterials, session: Session = Depends(get_session)):
    session.add(item)
    session.commit()
...
def update(item_id: int, new_data: BillOfMaterials, session: Session = Depends(get_session)):
    item = session.get(BillOfMaterials, item_id)
    if not item:
        raise HTTPException(status_code=404, detail="Not found")

...
def update_status(item_id: int, payload: StatusUpdateRequest, session: Session = Depends(get_session)):
    ...
    if item.status == payload.new_status:
        raise HTTPException(status_code=400, detail=f"Production order is already in status: {payload.new_status}")

    # update sub-orders
    sub_orders = session.query(ProductionOrder).filter(ProductionOrder.parentProductionOrderDetailsID == item_id).all()
    for sub_order in sub_orders:
        update_production_order_status_backend(sub_order.ID, payload.new_status, session)
